...
import { parseManyProductsKaspi } from "./util/scrapers/parseManyProductsKaspi";
...
async function main() {
  try {
    ...
    const formattedUrls = urls.map((url) => `${baseUrl}${url}${merchantCode}`);
