...
// Load the Excel file
const filePath = path.join(__dirname, "leroy_data", "test_data2.xlsx");
const workbook = XLSX.readFile(filePath);
const sheet = workbook.Sheets[workbook.SheetNames[0]];

// Create a new Excel workbook and worksheet
const excelWorkbook = new ExcelJS.Workbook();
const excelWorksheet = excelWorkbook.addWorksheet("With Lemana Info");

const outputFilePath = path.join(folderPath, "lemana_data.xlsx");
...
async function getImageUrl(page, itemCode) {
  ...
  try {
    ...
    // No retries
    await page.goto(`https://lemanapro.ru/search/?q=${itemCode}`, {
      waitUntil: "networkidle2",
      timeout: 10000,
    });

    await page.waitForSelector("div[data-qa-product]", { timeout: 7000 });

    await page.click("div[data-qa-product] a");

    // Wait for necessary elements to load
    try {
      await page.waitForSelector('div[data-qa="desktop-media-gallery"]', {
        timeout: 7000,
      });
      await page.waitForSelector('div[data-qa="section-content-vlimited"]', {
        timeout: 7000,
      });
    ...
    const info = await page.evaluate(() => {
      ...
      const descriptionDiv = document.querySelector(
        'div[data-qa="section-content-vlimited"]'
      );
      if (descriptionDiv) {
      ...
      const galleryDiv = document.querySelector(
        'div[data-qa="desktop-media-gallery"]'
      );
...
async function processItemCodes(itemCodes) {
  ...
  // ARGS - important to work with Lemana, otherwise it doesn't work
  ...
  for (const itemCode of itemCodes) {
    ...
    try {
      ...
      leroyInfo.push([itemCode, description, imgLinks]);
      console.log(
